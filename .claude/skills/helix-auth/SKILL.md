...
HELIX_OAUTH_CLIENT_SECRET=<client secret>   # omit entirely for public clients
...
the one workspace this app instance is locked to. **Do not** map Helix
...
3. Check whether `HELIX_OAUTH_TENANT` (the app's own configured slug) appears
   among the returned `slug` values:
   ...
   - **No match** → respond `403` (the app's own response — this isn't
     something Helix returns) and **discard the tokens immediately**. No
     session is created, no user record is created or updated, nothing is
     persisted.

If the app is already verifying the `id_token` for other reasons, its
`helix_tenant_slug` claim (see the claims table above) carries the same
workspace the token was bound to and can serve as a lighter-weight signal than
this API call — but it's a snapshot of what was requested at authorize time,
not live membership, so `/api/v1/tenants/` remains the recommended check for
this pattern.
